...
def tif_2_array_window_bands(tif_file, window, band):
    tif_open = rasterio.open(tif_file)
    tif_array = tif_open.read(band, window=window)
    tif_open.close()
    return tif_array
...
def write_reprojected_array_2_tif_bands(red, green, blue, new_tif_name_full_path, kwargs_tif):
    if new_tif_name_full_path.exists():
        os.remove(new_tif_name_full_path)
    reprojected_tif = rasterio.open(new_tif_name_full_path, 'w', **kwargs_tif)
    reprojected_tif.write(red.astype('float32'), indexes=1)
    reprojected_tif.write(green.astype('float32'), indexes=2)
    reprojected_tif.write(blue.astype('float32'), indexes=3)
    reprojected_tif.close()
...
def reproject_tif_array_w_array(tif_file_base, tif_file_2_reproject_array, tif_file_2_reproject_transform):
    width_ = get_tif_width(tif_file_base)
    height_ = get_tif_height(tif_file_base)
    base_crs = get_tif_crs(tif_file_base)
    base_transform = get_tif_transform(tif_file_base)
    array_2_reproject = tif_file_2_reproject_array
    transform_2_reproject = tif_file_2_reproject_transform
    reprojected_array = np.zeros((height_, width_))
    reproject(
        array_2_reproject,
        reprojected_array,
