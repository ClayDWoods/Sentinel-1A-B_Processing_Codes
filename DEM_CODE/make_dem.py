...
class combine_all_dem_tifs():
    def __init__(self):
        ...
        self.combined_linear_path_dem_match = self.dem_path / 'linear_rates_combined_all_dem_crop.tif'
        self.combined_coherence_path = self.combined_pairs_base_path / 'coherence_combined_all.tif'
        self.combined_coherence_path_dem_match = self.dem_path / 'coherence_combined_all_dem_crop.tif'
    ...
    def combine_arrays(self):
        ...
        combiend_coherence_array = rasterio_basic_functions.tif_2_array(self.combined_coherence_path)
        ...
        final_dem_array_mask = np.where(final_dem_array != 0,
                                        1,
                                        0)
        combined_linear_array_dem_mask = np.multiply(final_dem_array_mask, combined_linear_array)
        combined_coherence_array_dem_mask = np.multiply(final_dem_array_mask, combiend_coherence_array)
        ...
        rasterio_basic_functions.write_reprojected_array_2_tif(final_dem_array,
                                                               self.final_dem_path,
                                                               final_kwargs)
        rasterio_basic_functions.write_reprojected_array_2_tif(combined_linear_array_dem_mask,
                                                               self.combined_linear_path_dem_match,
                                                               final_kwargs)
        rasterio_basic_functions.write_reprojected_array_2_tif(combined_coherence_array_dem_mask,
                                                               self.combined_coherence_path_dem_match,
                                                               final_kwargs)
