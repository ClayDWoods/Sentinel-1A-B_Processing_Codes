...
class insar_processing_snap():
    ...
    Orbit_Keys = ['orbit_degree']
    Orbit_Degree = 3  # 1, 2, 3, 4 or 5
    Orbit_Vals = [Orbit_Degree]
    #
    # 4.
    # -------------------------------------------------------------------------------------------------------------------------------
    ...
    burststart = [str(1)]
    burstend = [str(9)]
    topsar_split_swaths_vals = [['IW3'], burststart, burstend]  # list of swaths to use IW123, IW12, IW23, IW1, IW2, IW3
    #
    # 5.
    # -------------------------------------------------------------------------------------------------------------------------------
    ...
    def __init__(self, sys_argv_int, param_dict=all_Parameters):
        ...
        self.orbit_degree = str(self.param_dict['orbit_degree'])
    ...
    def make_interferogram(self):
        ...
        for esd_path, swath in zip(self.esd_paths, self.swaths):
            o_type = ('-PsrpPolynomialDegree=' + str(self.srp_polynomial_degree) + ' ' +
                      ...
                      '-PorbitDegree=' + self.orbit_degree + ' ' +
    ...
    def TopoPhaseRemoval(self):
        ...
        o_type = ('-PorbitDegree=' + self.orbit_degree + ' ' +
                  self.dem_p + ' ')
        ext_name = 'TPR'
        out = ('-t ' + str(self.product_output_directory / (self.pair_dates + '_' +
                                                            ext_name)) + ' ')
    ...
    def Terrain_Correction(self):
        ...
        tc_command2 = self.base_snap_exe + ao_flag + out2 + self.number_of_processors + o_type2 + source2
    ...
    def img_2_geotiff(self):
        ...
        images_2_tiff_list = [displacement_2_tiff, dem_2_tiff, local_incidence_2_tiff, coh_image_2_tiff,
    ...
    def clean_tif_borders(self):
        ...
        rc.coherence_mask(self.coherence_path_tif, self.local_incidence_path_tif)
